...
namespace WebPDRSystem.Controllers
{
    public class PDRAPI : ControllerBase
    {
        ...
        public async Task<ActionResult> SavePdr(string urls)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                httpClient.Timeout = new TimeSpan(1,30,0);
                using (var response = await httpClient.GetAsync(url))
                {
                    using (var content = response.Content)
                    {
                        ...
                        foreach (var item in root)
                        {
                            ...
                            if (item.InterviewedByNavigation != null)
                                item.InterviewedByNavigation = FindUser(item.InterviewedByNavigation);
                            ...
                            foreach (var meds in item.PatientNavigation.Medications)
                            {
                                ...
                                meds.SignatureNurseNavigation = FindUser(meds.SignatureNurseNavigation);
                            }
                            ...
                            foreach (var orders in item.DoctorOrders)
                            {
                                ...
                                {
                                    orders.SignatureNavigation = FindUser(orders.SignatureNavigation);
                                }
                            }
                            foreach (var qd in item.Qdform)
                            {
                                ...
                                qd.PdrId = 0;
                                qd.SignatureOfQd = 0;
                                qd.SignatureOfQdNavigation = FindUser(qd.SignatureOfQdNavigation);
                            }
                            foreach (var qn in item.Qnform)
                            {
                                ...
                                qn.PdrId = 0;
                                qn.SignatureOfQn = 0;
                                qn.SignatureOfQnNavigation = FindUser(qn.SignatureOfQnNavigation);
                            }
                        }
        ...
        public Pdrusers FindUser(Pdrusers user)
        {
            var usr = _context.Pdrusers.FirstOrDefault(x =>
